...
typedef struct _RTL_RELATIVE_NAME {
	...
	PUNICODE_STRING CurrentDirectory;	//Win7由RtlpReferenceCurrentDirectory返回
...
typedef struct _PEB
{
	...
	//Private Extension
	FLS_CALLBACK_INFO* FlsCallback;	//Ext:0x20c Win7:0x20c
	LIST_ENTRY FlsListHead;	//Ext:0x210 Win7:0x210
	RTL_BITMAP* FlsBitmap;	//Ext:0x218 Win7:0x218
	ULONG FlsBitmapBits[4];	//Ext:0x21c Win7:0x21c
	ULONG FlsHighIndex;		//Ext:0x22c Win7:0x22c
} PEB;
...
typedef struct _TEB
{
	...
	//TEB和PEB内存上限为4KB
	//Private Extension
	//现在有点问题，没法在创建线程时初始化teb，还好默认值为0
	//但是HardErrorMode应该继承自Process的ErrorMode
	//如果以后能处理tls问题，肯定会控制线程创建，到时候会解决
	PVOID FlsData;	//Ext:0xfb8 Win7:0xfb4
	UCHAR HasFiberData;	//Ext:0xfbc Win7:0xfca pos2
	ULONG HardErrorMode;	//Ext:0xfc0 Win7:0xf28 
	UCHAR ExtPad[3];
} TEB;
