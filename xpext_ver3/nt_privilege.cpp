...
{
	NTSTATUS Result=NtOpenThreadTokenEx(NtCurrentThread(),DesiredAccess,TRUE,OBJ_KERNEL_HANDLE,TokenHandle);
	if (!NT_SUCCESS(Result))
		Result=NtOpenThreadTokenEx(NtCurrentThread(),DesiredAccess,FALSE,OBJ_KERNEL_HANDLE,TokenHandle);
	return Result;
}
                    ...
                    OUT PVOID *ReturnedState)
{
    ...
    if (!State->Token)	//IsImpersonating为FALSE时，Token必然为NULL，流程与汇编一致
    {
        ...
        else
        {
             ...
             */
            Status = NtOpenProcessTokenEx(NtCurrentProcess(), TOKEN_DUPLICATE, OBJ_KERNEL_HANDLE, &ProcessToken);
